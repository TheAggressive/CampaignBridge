...
class NoDirectWordPressStorageFunctionsRule implements Rule {
	...
	/**
	 * Functions allowed in legacy code during migration.
	 *
	 * @var array<string>
	 */
	private const ALLOWED_IN_LEGACY = array(
	...
	public function processNode( Node $node, Scope $scope ): array {
		...
		// Allow in legacy directories during migration
		foreach ( self::ALLOWED_IN_LEGACY as $legacyPath ) {
			if ( str_starts_with( $relativePath, $legacyPath ) ) {
				return array();
			}
		}
