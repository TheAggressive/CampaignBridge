...
- build tooling for blocks (`@wordpress/scripts`, `@wordpress/create-block`)
...
1. Confirm the repository root and read `CLAUDE.md` plus `docs/email-block-architecture.md`.
...
Use CampaignBridge's existing scripts:

- `pnpm build:blocks` for compiled block assets
- `pnpm qa:fast` for the pre-push quality gate
- Native PHPUnit fixtures for PHP tests; CampaignBridge does not use `wp-env`
