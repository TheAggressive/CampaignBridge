...
function runWp(cmdArgs, { pathArg, urlArg, allowRoot }) {
  ...
  return {
    ...
    status: out.status,
    error: out.error
      ? { message: out.error.message, code: out.error.code }
      : null,
...
function main() {
  ...
  if (!info.ok) {
    report.notes.push(
      'WP-CLI not available on PATH. Install WP-CLI or run inside the intended container/environment.'
    );
  ...
  if (!isInstalled.ok) {
    report.notes.push(
      'WordPress not detected at the given path/url. Check --path/--url (multisite) and that wp-config.php is present.'
    );
    process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
    return;
  }
